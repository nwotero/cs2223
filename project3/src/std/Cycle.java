...
import project3.EdgeWeightedGraph;
import project3.Edge;
...
public class Cycle {
    ...
    private boolean hasSelfLoop(EdgeWeightedGraph G) {
        for (int v = 0; v < G.V(); v++) {
            for (int w : G.adj(v)) {
                if (v == w) {
    ...
    private boolean hasParallelEdges(EdgeWeightedGraph G) {
        marked = new boolean[G.V()];
